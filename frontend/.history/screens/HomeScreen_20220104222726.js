...
export default class HomeScreen extends Component {
    ...
    render() {
        ...
        var userRef = firestore().collection('favoritelist').doc(user.uid).collection(user.uid).doc('9Nz9qe3BLRzvZKk2VMmb');
          ...
          try {
            ...
            if (doc.exists) {
              ...
              this.setState({marker:{lng:doc.data().lat}});
              this.setState({marker:{lat:doc.data().lng}});
              console.log('success')
            } else {
      ...
      */
      ...
      const users = firestore().collection('favoritelist').doc(user.uid).collection(user.uid).doc('9Nz9qe3BLRzvZKk2VMmb').get();
      console.log(users)
      userData()

      const data = this.state.marker
      console.log(data)
